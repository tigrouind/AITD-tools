namespace VarsViewer
{
	public static class Objects
	{
		...
		[
			...
			new() { Name = "Flags", Type = ColumnType.FLAGS, Offset = 4, Values = new() { { 1, "anim" }, { 4, "draw" }, { 8, "back" }, { 16, "push" }, { 64, "trig" }, { 128, "pick" }, { 256, "fall" } } },
			...
			new() { Name = "ZVType", Offset = 6, Values = new() { { 0, "none" }, { 1, "normal" }, { 2, "cube" }, { 3, "rot" }, { 4, "collider" } } },
			new()
			{
				Name = "Angle",
				Columns = [
			...
			new()
			{
				...
				Columns = [
					...
					new() { Name = "Mode", Offset = 32, Values = new() { { 0, "floor" }, { 1, "room" }, { 2, "camera" } }, IncludeZero = true, Condition = 34 }
				]
			},
			...
			new()
			{
				...
				Columns = [
					...
					new() { Name = "Type", Offset = 42, Values = new() { { 0, "once" }, { 1, "repeat" }, { 2, "uninterrupt" } }, IncludeZero = true, Condition = 38 },
			...
			new()
			{
				...
				Columns = [
					...
					new() { Name = "Mode", Offset = 46, Values = new() { { 0, "none" }, { 1, "manual" }, { 2, "follow" }, { 3, "track" } }, Condition = 48 }
				]
			},
			new()
			{
				...
				Columns = [
					...
					new() { Name = "Flag", Type = ColumnType.FLAGS, Offset = 12, Values = new() { { 1, "use" }, { 2, "eat" }, { 4, "read" }, { 8, "reload" }, { 16, "fight" }, { 32, "jump" }, { 64, "open" }, { 128, "close" }, { 256, "push" }, { 512, "throw" }, { 1024, "drop" } } },
