	...
	public static class Actors
	{
		...
		[
			...
			new() {
				Name = "Flags",
				Columns = [
					new() { Type = ColumnType.FLAGS, Offset = 4, Values = new() { { 1, "anim" }, { 4, "draw" }, { 8, "back" }, { 16, "push" }, { 32, "special" }, { 64, "trig" }, { 128, "pick" }, { 256, "fall" } } },
					new() { Name = "Col", Offset = 6, Values = new() { { 0, "N" }, { 1, "Y" } } }
				]
			},
		...
		Name = "Room",
			...
			new()
			{
				...
				Columns = [
					...
					new() { Name = "Dir", Offset = 114, Values = new() { { -1, "▼" }, { 32767, "▼" }, { 1, "▲" } }, Condition = 2 },
			...
			new()
			{
				...
				Columns = [
					...
					new() { Name = "Mode", Offset = 50, Values = new() { { 0, "floor" }, { 1, "room" }, { 2, "camera" } }, IncludeZero = true, Condition = 52 }
				]
			},
			...
			new()
			{
				...
				Columns = [
					...
					new() { Name = "Type", Offset = 64, Values = new() { { 0, "once" }, { 1, "repeat" }, { 2, "uninter" } }, IncludeZero = true, Condition = 62 },
			...
			new()
			{
				...
				Columns = [
					...
					new() { Name = "Type", Offset = 70, Values = new() { { 0, "once" }, { 1, "repeat" }, { 2, "uninter" } }, Condition = 68 },
			...
			new() { Name = "Speed", Offset = 116, Values = new() { { -1, "back" }, { 0, "idle" }, { 1, "walk" }, { 2, "walk" }, { 3, "walk" }, { 4, "walk" }, { 5, "run" } } },
			new() { Name = "Fall", Offset = 104, Values = new() { { 0, "N" }, { 1, "Y" } } },
			new()
			{
				Name = "Chrono",
				Columns = [
			...
			new()
			{
				...
				Columns = [
					...
					new() { Name = "Mode", Offset = 82, Values = new() { { 0, "none" }, { 1, "manual" }, { 2, "follow" }, { 3, "track" } } },
			...
			new()
			{
				...
				Columns = [
					...
					new() { Name = "Type", Offset = 142, Values = new() { { 0, "none" }, { 1, "pre_hit" }, { 2, "hit" }, { 3, "unknown" }, { 4, "pre_fire" }, { 5, "fire" }, { 6, "pre_throw" }, { 7, "throw" }, { 8, "hit_obj" }, { 9, "during_throw" }, { 10, "pre_hit" } } },
