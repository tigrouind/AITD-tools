...
class Program
{
	...
	static void Main(string[] args)
	{
		...
		while (!quit)
		{
			...
			void UpdateTitle()
			{
				string formatTime(TimeSpan time) => $"{time:mm\\:ss}.{(int)Math.Floor(time.Milliseconds / (1000.0 / 60.0)):D2}";
				var newTitle = string.Empty;
				if (movie.IsLoaded)
				{
					newTitle = $"{formatTime(movie.CurrentTime)} / {formatTime(movie.TotalTime)}";
				}
				else if (movie.IsRecording)
				{
					newTitle = $"{formatTime(movie.CurrentTime)}";
				}
